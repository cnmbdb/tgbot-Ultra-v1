...
CREATE INDEX IF NOT EXISTS idx_energy_ai_bishu_is_active ON public.t_energy_ai_bishu (is_active);

--
-- Name: idx_energy_ai_bishu_last_energy; Type: INDEX; Schema: public; Owner: -
-- 最后下发能量时间索引（用于判断1小时未使用自动停用）
--

CREATE INDEX IF NOT EXISTS idx_energy_ai_bishu_last_energy ON public.t_energy_ai_bishu (last_energy_time) WHERE last_energy_time IS NOT NULL;
...
DO $$
BEGIN
    PERFORM * FROM pg_extension WHERE extname = 'pg_row_security';
    IF FOUND THEN
        EXECUTE format('ALTER TABLE %I.%I RESET row_security', 'public', 'energy_platform_package');
    END IF;
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'unrestrict skipped: %', SQLERRM;
END $$;

-- Fix sequences automatically after database initialization
DO $$
DECLARE
    r RECORD;
    seq_name TEXT;
...
BEGIN
    ...
    LOOP
        ...
        IF seq_name IS NOT NULL THEN
            BEGIN
                EXECUTE format('SELECT COALESCE(MAX(%I), 1) FROM public.%I', r.column_name, r.table_name) INTO max_val;
                PERFORM setval(seq_name::regclass, max_val, true);
            EXCEPTION WHEN OTHERS THEN
                -- Ignore errors for individual sequences so the loop can continue
            END;
        END IF;
    END LOOP;
END $$;
