...
--
-- ============================================================================
-- 兼容历史无前缀表名：创建 VIEW 指向 t_ 前缀表（原 002_views_no_prefix.sql 内容）
-- 为了只保留一个初始化脚本，这里直接内联视图创建逻辑
-- ============================================================================
--
...
BEGIN
  -- 定义需要创建视图的表名映射（表名 -> 视图名）
  FOR v_table_name, v_view_name IN
    SELECT * FROM (VALUES
      ('t_transit_wallet', 'transit_wallet'),
      ('t_transit_wallet_black', 'transit_wallet_black'),
      ('t_transit_wallet_coin', 'transit_wallet_coin'),
      ('t_transit_wallet_trade_list', 'transit_wallet_trade_list'),
      ('t_transit_user_wallet', 'transit_user_wallet'),
      ('t_monitor_wallet', 'monitor_wallet'),
      ('t_collection_wallet', 'collection_wallet'),
      ('t_collection_wallet_list', 'collection_wallet_list'),
      ('t_premium_platform', 'premium_platform'),
      ('t_premium_platform_order', 'premium_platform_order'),
      ('t_premium_platform_package', 'premium_platform_package'),
      ('t_premium_wallet_trade_list', 'premium_wallet_trade_list'),
      ('t_fms_recharge_order', 'fms_recharge_order'),
      ('t_fms_wallet_trade_list', 'fms_wallet_trade_list'),
      ('t_telegram_bot_ad', 'telegram_bot_ad'),
      ('t_telegram_bot_ad_keyboard', 'telegram_bot_ad_keyboard'),
      ('t_telegram_bot_user', 'telegram_bot_user'),
      ('t_energy_platform_package', 'energy_platform_package')
    ) AS t(table_name, view_name)
  LOOP
    -- 检查表是否存在
    IF to_regclass('public.' || v_table_name) IS NOT NULL THEN
      BEGIN
        -- 使用 CREATE OR REPLACE VIEW 确保幂等性，即使视图已存在也能更新
        v_sql := 'CREATE OR REPLACE VIEW public.' || quote_ident(v_view_name) ||
                 ' AS SELECT * FROM public.' || quote_ident(v_table_name);
        EXECUTE v_sql;
      ...
      EXCEPTION
        WHEN OTHERS THEN
          ...
          RAISE WARNING 'Failed to create view %: % (SQLSTATE: %)',
            v_view_name, SQLERRM, SQLSTATE;
      END;
    ELSE
      RAISE NOTICE 'Table % does not exist, skipping view %', v_table_name, v_view_name;
    END IF;
  END LOOP;

  -- 输出总结信息
  IF v_error_count > 0 THEN
    RAISE WARNING 'View creation completed with % error(s). Please check the logs above.', v_error_count;
  ELSE
