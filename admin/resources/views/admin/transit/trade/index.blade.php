...
@section('nav-status-transit-trade', 'active')
@section('contents')
    <div class="row">
        <div class="col-lg-12">
            <div class="wrapper wrapper-content animated fadeInUp">
                ...
                <div style="margin-top:1px;margin-left:10px;color:red">
                    1. 补发：只有处理状态为-余额不足，才能补发<p>
                    2. 禁止：只有处理状态为-余额不足或者待兑换，才能禁止
                </div>

                <div class="project-list">
                    <table class="layui-table"
                           lay-data="{url:'{{route("admin.transit.trade.get_data")}}',page:true,id:'userTable',loading:true}"
                        ...
                        <thead>
                            <tr>
                                ...
                                <th lay-data="{field:'coin_name',align:'center',width:100}">转入币名</th>
                                <th lay-data="{field:'amount',align:'center',width:120}">转入数额</th>
                                <th lay-data="{field:'sendback_coin_name',align:'center',width:100}">回款币名</th>
                                <th lay-data="{field:'sendback_amount',align:'center'}">回款数额</th>
                                ...
                                <th lay-data="{field:'sendback_time',align:'center'}">出款时间</th>
...
@section('scripts')
    ...
    <script type="text/html" id="tpl_opt">
        <div class="layui-btn-group">
            @if( auth('admin')->user()->can('兑换补发') || auth('admin')->user()->hasrole('超级管理员') )
                <button class="layui-btn layui-btn-sm" onclick="javascript:tools_add('兑换补发',
                    {
                        'rid':['','hidden','@{{ d.rid }}'],
                        ...
                        'process_comments':['处理备注','span','@{{d.process_comments}}'],
                    },
                    '{{route("admin.transit.trade.reswap")}}',get_online_data);">补发
                </button>
            @endif
            @if( auth('admin')->user()->can('禁止兑换') || auth('admin')->user()->hasrole('超级管理员') )
                <button class="layui-btn layui-btn-sm delete_button" onclick="javascript:tools_add('禁止兑换',
                    {
                        'rid':['','hidden','@{{ d.rid }}'],
                        'tx_hash':['交易hash','span','@{{d.tx_hash}}'],
                        'transferfrom_address':['发送钱包地址','span','@{{d.transferfrom_address}}'],
                    ...
                    '{{route("admin.transit.trade.stopswap")}}',get_online_data);">禁止
                </button>
            @endif
        </div>
    </script>
