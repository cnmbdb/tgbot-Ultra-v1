...
@section('nav-status-premium-package', 'active')
@section('contents')
    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="wrapper wrapper-content animated fadeInUp">
                ...
                @if( auth('admin')->user()->can('添加会员套餐') || auth('admin')->user()->hasrole('超级管理员') )
                    <button class="layui-btn layui-btn-sm layui-btn-normal" style="margin-top:1px;margin-left:10px;" onclick="javascript:tools_add('添加会员套餐',
                                {
                                'premium_platform_rid':['会员平台ID','select',[PlatformName],''],
                                'package_name':['套餐名称','text',''],
                                'package_month':['套餐月份','select',[PackageMonth],''],
                                'usdt_price':['USDT售价','text','','必须大于0'],
                                'show_notes':['显示说明','textarea','',''],
                                'seq_sn':['排序','text','','数字越大越靠前']
                                },
                                '{{route("admin.premium.package.add")}}',get_online_data)">添加会员套餐
                    </button>
                @endif
                
                @if( auth('admin')->user()->can('复制会员套餐') || auth('admin')->user()->hasrole('超级管理员') )
                    <button class="layui-btn layui-btn-sm export_button" style="margin-top:1px;margin-left:10px;" onclick="javascript:tools_add('复制会员套餐',
                                {
                                'paste_premium_platform_rid':['覆盖会员平台ID','select',[PlatformName],''],
                                'copy_premium_platform_rid':['来源会员平台ID','select',[PlatformName],'']
                                },
                                '{{route("admin.premium.package.copy_paste")}}',get_online_data)">复制会员套餐
                    </button>
                @endif
                
                <div style="margin-top:1px;margin-left:10px;color:red">
                    1. 会员套餐可针对不同会员平台设置不同的会员套餐<p>
                    2. 会员套餐仅支持USDT售价方式
                </div>

                <div class="project-list">
                    <table class="layui-table"
                           lay-data="{url:'{{route("admin.premium.package.get_data")}}',page:true,id:'userTable',loading:true}"
                        ...
                        <thead>
                            <tr>
                                ...
                                <th lay-data="{field:'premium_platform_rid',align:'center'}">会员平台ID</th>
                                <th lay-data="{field:'package_name',align:'center'}">套餐名称</th>
                                <th lay-data="{field:'package_month_val',align:'center'}">套餐月份</th>
...
@section('scripts')
    ...
    <script type="text/html" id="tpl_opt">
        <div class="layui-btn-group">
            @if( auth('admin')->user()->can('修改能量套餐') || auth('admin')->user()->hasrole('超级管理员') )
                <a href ="{{route('admin.premium.package.show')}}?rid=@{{ d.rid }}" class="layui-btn layui-btn-sm edit_button">修改</a>
            @endif
            ...
            @if( auth('admin')->user()->can('删除能量套餐') || auth('admin')->user()->hasrole('超级管理员') )
                <button class="layui-btn delete_button layui-btn-sm" onclick="javascript:confirm_opt(
                    ()=>{
                        form_func('{{route("admin.premium.package.delete")}}',{'rid':'@{{d.rid}}'},get_online_data);
                    });">删除
                </button>
            @endif
        </div>
    ...
    <script>
        ...
        var PackageMonth = @json($PackageMonth, JSON_UNESCAPED_UNICODE);
        ...
        layui.use(['table', 'layer'], function () {
            ...
            form.on('switch(status)', function (obj) {
                confirm_opt(
                    () => {
                        form_func('{{route("admin.premium.package.change_status")}}', {
                            'rid': obj.elem.id,
                            'status': obj.value,
