...
@section('nav-status-energy-trade', 'active')
@section('contents')
    <div class="row">
        <div class="col-lg-12">
            <div class="wrapper wrapper-content animated fadeInUp">
                ...
                <div style="margin-top:1px;margin-left:10px;color:red">
                    1. 只记录转入数额大于等于1 TRX的交易，低于1 TRX的交易不记录<p>
                    2. 补发：只有处理状态为-失败，才能补发<p>
                </div>

                <div class="project-list">
                    <table class="layui-table"
                           lay-data="{url:'{{route("admin.energy.trade.get_data")}}',page:true,id:'userTable',loading:true}"
        ...
        <div class="layui-btn-group">
            @if( auth('admin')->user()->can('兑换补发') || auth('admin')->user()->hasrole('超级管理员') )
                <button class="layui-btn layui-btn-sm" onclick="javascript:tools_add('兑换补发',
                    ...
                    '{{route("admin.energy.trade.reorder")}}',get_online_data);">补发
                </button>
            @endif
        </div>
    </script>
