...
@section('contents')
    <div class="widget-body">
        <div class="widget-main" style="padding:0;">
            ...
            @endsection
            <div class="form-box">
                <form class="layui-form" action="{{route('admin.telegram.keyreply.update')}}"  id="add_news" name="add_news"  method="POST">
                    <div class="layui-form-item">
                        <label class="layui-form-label">机器人token：</label>
                        <div class="layui-col-md5" style="margin-top:10px">
                            <span >{{$data->bot_token}}</span>
                        </div>
                    </div>
                    <div class="layui-form-item">
                        <label class="layui-form-label">机器人显示名称：</label>
                        <div class="layui-col-md5"  style="margin-top:10px">
                            <span >{{$data->bot_firstname}}</span>
                        </div>
                    </div>
                    ...
                    <div class="layui-form-item">
                            ...
                            <div class="layui-col-md5">
                                <input type="text" name="monitor_word"  autocomplete="off" class="layui-input" value="{{$data->monitor_word}}">
                            </div>
                    </div>
                    ...
                    <div class="layui-form-item">
                            ...
                            <div class="layui-col-md5">
                                <textarea name="reply_content"  autocomplete="off" class="layui-textarea">{{$data->reply_content}}</textarea>
                            </div>
                    </div>
                    <div class="layui-form-item">
                        <label class="layui-form-label">关键字类型：</label>
                        <div class="layui-col-md5">
                            <select name="key_type" lay-filter="key_type"  id="key_type" lay-verify="required">
                                @foreach ($KeyreplyKeytype as $k => $val)
                                    <option value="{{$k}}" @if ($data->key_type == $k) selected @endif>{{$val}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="layui-form-item">
                        ...
                        <div class="layui-col-md5">
                            <select name="opt_type" lay-filter="opt_type"  id="opt_type" lay-verify="required">
                                @foreach ($KeyreplyOpttype as $k => $val)
                                    <option value="{{$k}}" @if ($data->opt_type == $k) selected @endif>{{$val}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
...
@section('scripts')
    ...
    <script>
                ...
                $("#add_news").ajaxSubmit({
                    ...
                    'success':function(res){
                        ...
                        if (res.code == 200) {
                            ...
                            setTimeout(() => {
                                location.href= '{{route("admin.telegram.keyreply.index")}}';
                            }, 1000);
                            return false;
                        } else {
        ...
        var replyphoto = '{{$data->reply_photo}}';
        if(replyphoto != ''){
           $(".thumb-box").css({
                'background-image':'url({{$data->reply_photo}})',
                'border':'none'
            }); 
        }
