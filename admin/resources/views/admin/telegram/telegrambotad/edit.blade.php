...
@section('contents')
    <div class="widget-body">
        <div class="widget-main" style="padding:0;">
            ...
            @endsection
            <div class="form-box">
                <form class="layui-form" action="{{route('admin.telegram.telegrambotad.update')}}"  id="add_news" name="add_news"  method="POST">
                    ...
                    <div class="layui-form-item">
                        <label class="layui-form-label">通知周期：</label>
                        <div class="layui-col-md5">
                            <select name="notice_cycle" lay-filter="notice_cycle"  id="notice_cycle" lay-verify="required">
                                @foreach ($NoticeCycle as $k => $val)
                                    <option value="{{$k}}" @if ($data->notice_cycle == $k) selected @endif>{{$val}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="layui-form-item">
                            ...
                            <div class="layui-col-md5">
                                <input type="text" name="notice_obj"  autocomplete="off" class="layui-input" value="{{$data->notice_obj}}">
                            </div>
                    </div>
                    <div class="layui-form-item">
                    ...
                    <div class="layui-form-item">
                            ...
                            <div class="layui-col-md5">
                                <textarea name="notice_ad"  autocomplete="off" class="layui-textarea">{{$data->notice_ad}}</textarea>
                            </div>
                    </div>
                    <div class="layui-form-item">
...
@section('scripts')
    ...
    <script>
                ...
                $("#add_news").ajaxSubmit({
                    ...
                    'success':function(res){
                        ...
                        if (res.code == 200) {
                            ...
                            setTimeout(() => {
                                location.href= '{{route("admin.telegram.telegrambotad.index")}}';
                            }, 1000);
                            return false;
                        } else {
        ...
        var noticephoto = '{{$data->notice_photo}}';
        if(noticephoto != ''){
           $(".thumb-box").css({
                'background-image':'url({{$data->notice_photo}})',
                'border':'none'
            }); 
        }
