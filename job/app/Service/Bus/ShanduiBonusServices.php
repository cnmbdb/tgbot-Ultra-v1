    ...
    public function handleGrant($data){
        ...
        foreach ($data as $k => $v) {
            if(empty($transitWalletCoinList[$v->transit_wallet_id.strtolower($v->coin_name)])){
                ...
                if($coin_data['exchange_rate'] > 10){
                    $save_data = [];
                    $save_data['process_status'] = 2;      //交易失败
                    $save_data['process_comments'] = '超过最大汇率10，当前：'.$coin_data['exchange_rate'];      //处理备注  
                    TransitWalletTradeList::where('rid',$v->rid)->update($save_data);
                    continue;
                }
                ...
                if($success_status == 1){
                    if($troncode == 200){
                        ...
                        $save_data['process_comments'] = '转帐成功,扣预支:'.$huan_yuzhi_amount.',汇率:'.$coin_data['exchange_rate'];      //处理备注  
