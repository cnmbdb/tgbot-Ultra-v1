...
## 🗺️ 当前开发计划（hotfix/bishutaocan）

### 已完成（本地分支）

1. **笔数套餐代理方式新增 `NL-API`**
   - 后台「能量管理 -> 机器人能量 -> 笔数套餐」支持选择 `提交到NL-API`。

2. **NL-API 笔数下发改为固定 1 小时 1 笔**
   - 笔数套餐下发参数固定为小时模式（1h/笔），不再按 1天/3天/30天策略派发。

3. **笔数套餐触发条件收敛为硬门槛**
   - 仅在以下 4 条同时满足时触发：
   - 已添加套餐地址（存在 `t_energy_ai_bishu` 记录）
   - 已开启笔数套餐（`is_open_bishu=Y`）
   - 已购买并有剩余笔数（`total_buy_usdt > 0` 且 `max_buy_quantity > total_buy_quantity`）
   - 当前地址能量 `< 65000`
   - 若地址能量 `>= 65000`，则不会触发下发。

### 验收建议

1. 在后台将机器人「笔数代理方式」设置为 `提交到NL-API`。
2. 用测试地址购买笔数套餐，确认 `max_buy_quantity` 增长。
3. 当地址能量 `>= 65000` 时，确认不触发下发。
4. 当地址能量 `< 65000` 时，确认 `job_handleaienergyorder` 触发并下发成功。

### 下一步（建议）

1. 增加“触发条件命中日志”与“未触发原因日志”（便于生产排查）。
2. 为 `NL-API` 增加接口失败重试与告警分级。
3. 为笔数套餐链路补一条端到端回归清单（配置、交易、触发、下发、通知）。
