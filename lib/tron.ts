...
export async function delegateAndUndelegate(
  ...
  try {
    ...
    // 如果有 permissionId，使用它来签名
    const signed = permissionId && permissionId > 0
      ? await tronWeb.trx.sign(transaction, cleanKey, permissionId)
      : await tronWeb.trx.sign(transaction, cleanKey);
