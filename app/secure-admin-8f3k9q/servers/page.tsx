...
export default function ServersPage() {
  ...
  return (
    <div>
      <h1 style={{ margin: '8px 0 16px', fontSize: 22, color: '#111827' }}>服务器列表</h1>
      <div style={{ fontSize: 13, color: '#6b7280', marginBottom: 10 }}>
        这里按来源主机/IP 汇总请求，便于快速定位哪个服务器在调用 API。
      </div>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {list.map((row, idx) => (
                <tr key={idx}>
                  <td style={td}>{row.source_host || '-'}</td>
                  <td style={td}>{row.source_ip}</td>
